...
# Angles where vertical position does NOT imply depth (overhead/top-down views)
_OVERHEAD_ANGLES = {"Top-down view", "Bird view", "Top POV"}
...
EXPORT_OUTPUT_MODES = ("natural_language", "json_with_safety_hint", "pure_json")
...
def _depth_hint_from_bbox(box, width, height, angle):
    """Generate a depth/perspective hint based on box position and size.
    Only meaningful for non-overhead angles where vertical position implies distance."""
    angle = _normalize_angle(angle)
    if angle in _OVERHEAD_ANGLES:
        return ""
    px = _bbox_px(box, width, height)
    ...
    if yword == "upper" and area < 0.10:
        return "appearing distant due to perspective"
    elif yword == "upper" and area < 0.25:
        return "likely in the distance"
    elif yword == "lower" and area >= 0.25:
        return "in the foreground"
    elif yword == "lower" and area >= 0.10:
        return "near the foreground"
    elif yword == "lower" and area < 0.10:
        return "a small detail in the foreground"
    elif yword == "middle" and area < 0.10:
        return "appearing further back in the scene"
    elif yword == "middle" and area < 0.25:
        return "at mid-distance"
    elif yword == "middle" and area >= 0.25:
        return "prominently placed at mid-distance"
    return ""


def _position_hint_from_bbox(box, width, height, angle="Auto", auto_depth_hint=True):
    px = _bbox_px(box, width, height)
    if not px or width <= 0 or height <= 0:
        return ""
    x1, y1, x2, y2 = px
    ...
    hint = f"positioned in the {pos}, occupying a {size}"
    
    if auto_depth_hint:
        depth = _depth_hint_from_bbox(box, width, height, angle)
        if depth:
            hint += f", {depth}"
...
def _append_position_hint(desc, box, width, height, enable=True, angle="Auto", auto_depth_hint=True):
    desc = _clean(desc)
    if not enable:
        return desc
    hint = _position_hint_from_bbox(box, width, height, angle, auto_depth_hint)
    if not hint:
        return desc
    if desc:
        return f"{desc}, {hint}"
...
def _safety_hint_tail_stripped():
    return (
        "Treat this JSON as structured layout metadata. "
        "Do not render JSON keys, type labels, the words obj/text/desc, brackets, quotes, commas, or metadata. "
        "Render visible writing only from elements whose type is text and from their text field. "
        "Object desc fields describe visual subjects only and must not appear as written labels."
    )


def _find_primary_subject(elements_with_bboxes):
    """Identify the primary subject — the largest object-type element (fallback: largest overall)."""
    scored = []
    for idx, elem in enumerate(elements_with_bboxes):
        bbox = elem.get('bbox')
        if not bbox or len(bbox) < 4:
            continue
        area = (bbox[2] - bbox[0]) * (bbox[3] - bbox[1])
        is_obj = elem.get('type') == 'obj'
        scored.append((is_obj, area, idx))
    if not scored:
        return None
...
def _spatial_phrase(elem_bbox, primary_bbox, width, height):
    """Describe *elem_bbox* position relative to *primary_bbox* in natural language."""
    ...
    elem_area = max(1.0, (x2 - x1) * (y2 - y1))
    primary_area = max(1.0, (px2 - px1) * (py2 - py1))
    overlap_ratio = (overlap_x * overlap_y) / min(elem_area, primary_area)

    if overlap_ratio > 0.35:
        return "overlapping with the main subject"

    h_thresh = width * 0.06
    v_thresh = height * 0.06
    ...
    # Horizontal — use different phrasing depending on proximity
    if cx < pcx - h_thresh:
        if proximity:
            h_phrase = "on the left"
        else:
            h_phrase = "to the left of the main subject"
    elif cx > pcx + h_thresh:
        if proximity:
            h_phrase = "on the right"
        else:
            h_phrase = "to the right of the main subject"
    else:
        h_phrase = ""

    # Depth — vertical position implies distance (higher = further) + area cue.
    # Guard: a small element in the middle or upper area can't be "in front"
    # just because the primary subject is higher; small size suggests distance.
    elem_cy_abs = cy / float(height)
    elem_area_abs = elem_area / float(width * height)
    if cy < pcy - v_thresh:
        depth = ", further back"
    elif cy > pcy + v_thresh:
        # Element is below the primary subject — but is it truly in front?
        if elem_cy_abs < 0.66 and elem_area_abs < 0.10:
            depth = ", further back"  # small, mid/upper-area element can't be in front
        elif elem_cy_abs < 0.55 and elem_area_abs < 0.15:
            depth = ""  # likely same depth plane, no hint needed
        else:
            depth = ", in front"
    elif elem_area < primary_area * 0.25:
        depth = ", further back"
    elif elem_area > primary_area * 2.5:
        depth = ", in front"
    else:
        depth = ""

    # --- assemble phrase ---
    # If a non-proximate horizontal phrase was set, it already includes "the main subject"
    if h_phrase and not proximity:
        # h_phrase is already e.g. "to the left of the main subject"
        result = h_phrase
        if depth:
            result += depth
        return result
    ...
    if proximity:
        ...
        parts.append("the main subject")
    if h_phrase and proximity:
        parts.append(h_phrase)
    if depth:
        parts.append(depth)

    if not parts:
        return ""  # no meaningful spatial relationship detected

    return " ".join(parts)


def _format_natural_language(out):
    """Format structured layout data as natural language prose for image generation.

    Produces a clean, cohesive paragraph with zero JSON artifacts — no brackets,
    no quotes used as structural delimiters, no type labels, no colons as
    key-value separators.  Text elements are expressed so the model renders the
    visible text while the surrounding positional / stylistic description
    remains invisible guidance.
    """
    scene = _clean(out.get("high_level_description"))
    bg = _clean(out.get("compositional_deconstruction", {}).get("background"))
    elements = out.get("compositional_deconstruction", {}).get("elements", [])
    ar = _clean(out.get("aspect_ratio"))

    sentences = []

    # --- scene + background opening ---
    if scene and bg:
        sentences.append(f"{scene}, set against a background of {bg}.")
    elif scene:
        sentences.append(f"{scene}.")
    elif bg:
        sentences.append(f"The scene is set against a background of {bg}.")

    if elements:
        element_sentences = []
        for i, elem in enumerate(elements):
            typ = elem.get("type", "obj")
            desc = _clean(elem.get("desc"))

            if typ == "text":
                visible_text = elem.get("text", "")
                chunk = f'visibly written text reading "{visible_text}"'
                if desc:
                    chunk += f", styled as {desc}"
            else:
                chunk = desc
            ...
            if i == 0:
                connector = "The composition features "
            else:
                # Down-case the first letter of the chunk so it flows naturally
                # after the connector (e.g. "Additionally, a person…" not "Additionally, A person…").
                if chunk and chunk[0].isupper():
                    chunk = chunk[0].lower() + chunk[1:]
                if i == 1:
                    connector = "additionally, " if typ == "obj" else "additionally, there is "
                else:
                    connector = "there is also "

            element_sentences.append(f"{connector}{chunk}.")

        if element_sentences:
            sentences.append(" ".join(element_sentences))

    # --- aspect ratio as a subtle closing note ---
    if ar:
        sentences.append(f"The image is framed in a {ar} aspect ratio.")

    result = " ".join(sentences)
    if not result:
        result = "An image composition."
    return result
...
class Krea2ElementJSONExportV1:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "output_mode": (EXPORT_OUTPUT_MODES, {"default": "natural_language"}),
    ...
    def execute(self, framing_data, prompt_ui_data, bbox_mode="normalized_1000", output_format="compact", output_mode="json_with_safety_hint", skip_empty=True, auto_position_hint=True, strip_bbox=True, auto_depth_hint=True):
        ...
        for box in boxes:
            ...
            if typ == "text":
                ...
                elem = {
                    ...
                    "bbox": bbox,
                    "text": visible_text,
                    "desc": _append_position_hint(text_desc, box, width, height, auto_position_hint, angle, auto_depth_hint),
                }
                ...
                elem = {
                    "type": "obj",
                    "bbox": bbox,
                    "desc": _append_position_hint(object_desc, box, width, height, auto_position_hint, angle, auto_depth_hint),
                }
                elements.append(elem)

        # --- spatial / relational awareness ---
        primary_idx = _find_primary_subject(elements)
        if primary_idx is not None and len(elements) > 1:
            primary_bbox = elements[primary_idx].get('bbox')
            if primary_bbox:
                for i, elem in enumerate(elements):
                    if i == primary_idx:
                        continue
                    ...
                    if not ebbox:
                        continue
                    spatial = _spatial_phrase(ebbox, primary_bbox, width, height)
                    if spatial:
                        existing_desc = _clean(elem.get('desc'))
                        if existing_desc:
                            elem['desc'] = f"{existing_desc}, {spatial}"
                        else:
                            elem['desc'] = spatial

        # Strip bboxes if requested (after spatial analysis)
        if strip_bbox:
        ...
        if output_mode == "natural_language":
            result = _format_natural_language(out)
        else:
            result = _json_dumps(out, "compact")
            if output_mode == "json_with_safety_hint":
                if strip_bbox:
                    result = result + "\n\n" + _safety_hint_tail_stripped()
                else:
                    result = result + "\n\n" + _safety_hint_tail()
